...
def escape_sieve_string(s):
    # Escape quotes and backslashes for Sieve double-quoted strings
    return s.replace("\\", "\\\\").replace('"', '\\"')

def find_user_sieve_dir(email_addr):
    # Check directory candidates: /home/vmail/<email>/sieve or /home/vmail/<localpart>/sieve
...
def main():
    ...
    if re.search(r"^(off|cancel|stop|關閉|取消|停用)", param_str, re.IGNORECASE):
        ...
        send_notification(
            ...
            f"您好：\n\n您的電子郵件自動回覆（Auto-Reply / Vacation）已於系統中成功停用。\n\n帳號：{from_addr}\n更新時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (UTC+8 台北時間)\n"
        )
        sys.exit(0)
    ...
    if re.search(r"^on(?:\s+(.*))?$", param_str, re.IGNORECASE):
        ...
        # Match date range formats: "2026-08-25 ~ 2026-08-30 [Title]"
    ...
    if not custom_subject:
        custom_subject = "休假中 / Out of Office"
    ...
    if is_always_on or not (start_dt and end_dt):
        ...
        time_desc = "即刻生效，直到寄信停用 (Always Active until turned off)"
    else:
    ...
    send_notification(
        from_addr,
        f"【自動回覆通知】已成功啟用自動回覆 (生效期間：{time_desc.split(' (')[0]})",
        notif_body
    )
